...
1. 使用 Tushare Pro 获取交易日线和复权因子，构造前复权 OHLC 数据；如 SDK 提供稳定的前复权便捷接口，可封装使用，但对外返回结构必须一致。
2. 为避免长周期指标在展示区间起点失真，实际抓取起始日早于用户所选起始日，至少预留 120 个交易日作为预热窗口；计算完成后再裁剪展示区间。
3. 数据按交易日期升序排列并去重。
4. 必须校验：必要字段、日期单调性、价格非负、成交量非负、OHLC 关系（`high >= max(open, close, low)` 且 `low <= min(open, close, high)`）、最少 60 条有效预热样本和最后交易日。少于 60 条时返回“数据不足”，60–119 条时允许分析但添加长周期指标可能不稳定的质量警告。
...
- KDJ 使用 9 日最高/最低，`RSV=(close-low9)/(high9-low9)×100`；分母为 0 时 RSV=50；K、D 以 50 为初值按 `2/3×前值+1/3×当前值` 递推，J=`3K-2D`，展示时不裁剪 J；
- True Range 为 `max(high-low, |high-prev_close|, |low-prev_close|)`；ATR14 使用与 RSI 相同的 Wilder RMA；
...
规则谓词进一步冻结如下：KDJ 交叉令 `diff=K-D`，上穿为交叉日 `diff>0` 且前一有效日 `diff<=0`，下穿反之；“最近 3 日”的阈值读取交叉日 K。5/10/20 日价格收益分别为 `close/close.shift(n)-1`；OBV 5 日斜率为 `(OBV-OBV.shift(5))/5`；量价背离比较 10 日价格收益与 `OBV-OBV.shift(10)` 的符号。任一方向量等于 0 时视为中性，不构成该方向命中或背离；当日上涨/下跌以 `close` 相对 `prev_close` 严格大于/小于定义，平盘为中性。

某条方向规则的全部输入非缺失时称为“可评估”，无论它最终命中偏多、偏空或保持中性；任一输入缺失时才不可评估。各组“可评估容量”是可评估方向规则的理论绝对分之和，分组原始分仅累加实际命中的带符号分。分组分=`原始分/可评估容量×分组权重`，并分别限制在趋势 `[-40,40]`、动量 `[-30,30]`、量价/波动 `[-30,30]`。某组可评估容量少于该组全部方向规则理论容量的 50% 时整组不可用。三组中至少两组可用且总可用权重不少于 60 才输出总分；总分为可用组分之和再乘 `100/可用权重`，并限制在 `[-100,100]`。因此“全部规则可评估但均中性”得到 0 分，而不是数据不足。
...
| 行情缓存 | SQLite | 成功取得并校验行情后 | 缓存键变化产生新记录 | 6 小时 TTL；历史截止日早于当前日时 30 天 TTL |
